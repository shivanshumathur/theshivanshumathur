...
export function retrieve(raw, limit = 3) {
  ...
  // Allowlist: if the question doesn't match anything in Shivanshu's corpus,
  // treat it as off-topic rather than letting it fall through to the LLM.
  if (!ranked.length) return { q, docs: [], refuse: true };
